...
#synthesis formulation with normalization to [0,1]
...
def vae_estimator_flex(hparams):   
#------------TODO INSERT HERE stage_i if not working--------------------------
    def estimator(A_val, y_batch_val, hparams):
        """Function that returns the estimated image"""
#        start = time.time()
        bs = utils_vaeflex.Binary_search(hparams.grid[-1],hparams.tol,hparams.flex_chosen)
        x_hat = []
        ...
        for i in bs:
            ...
            x_hat, z_hat = utils_vaeflex.stage_i(A_val,y_batch_val,hparams,i,init_obj,True)
        ...
        utils_vaeflex.setup_for_stage(init_obj,bs,z_hat)
        x_hat,_ = utils_vaeflex.stage_i(A_val,y_batch_val,hparams,final_i,init_obj,False)
        np.save(utils.get_checkpoint_dir(hparams, hparams.model_types[0])+'chosen_dim',i)
#        end = time.time()
#        duration = end-start
...
def k_sparse_wavelet_estimator(hparams): #pylint: disable = W0613
    ...
    def estimator(A_val, y_batch_val, hparams): #pylint: disable = W0613
        if hparams.measurement_type != 'project':
            raise RuntimeError
        y_batch_val_tmp = y_batch_val#/np.sqrt(hparams.n_input)
        x_hat_batch = []
        for y_val in y_batch_val_tmp:
            y_val_reshaped = np.reshape(y_val, [64, 64, 3])
#            print(y_val_reshaped.dtype)
#            plt.figure()
#            plt.imshow(y_val_reshaped*255)
#            plt.show()
...
#        for i in range(min(len(x_hat_batch),16)):
#            plt.figure()
#            print(x_hat_batch[i])
#            plt.imshow(np.reshape(x_hat_batch[i], [64, 64, 3])*255)#, interpolation="nearest", cmap=plt.cm.gray)
#            plt.show()
        return x_hat_batch
    return estimator
