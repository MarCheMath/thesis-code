...
def get_loss(hparams, x, logits, z_mean, z_log_sigma_sq):
    reconstr_losses = tf.reduce_sum(tf.nn.sigmoid_cross_entropy_with_logits(labels=x, logits=logits), 1)
    latent_losses = get_neg_KL(hparams,z_mean,z_log_sigma_sq)
#    latent_losses = -0.5 * tf.reduce_sum(1 + z_log_sigma_sq - tf.square(z_mean) - tf.exp(z_log_sigma_sq), 1) # negative KL divergence
    total_loss = tf.reduce_mean(reconstr_losses + hparams.weight_of_internal_distribution*latent_losses, name='total_loss')
    return total_loss


def get_z_var(hparams, batch_size):
    if hparams.n_z == -1:
        z = tf.Variable(tf.random_normal((batch_size, hparams.grid[-1]),stddev=hparams.stdv,mean=hparams.mean), name='z')
    else:
        z = tf.Variable(tf.random_normal((batch_size, hparams.n_z),stddev=hparams.stdv,mean=hparams.mean), name='z')
#    z = tf.Variable(np.random.normal(size=(batch_size, hparams.n_z),scale=hparams.stdv,loc=hparams.mean).astype('float32'), name='z')
#    if hparams.tf_init:
#        z = tf.Variable(tf.random_normal((batch_size, hparams.n_z),stddev=hparams.stdv,mean=hparams.mean), name='z')
#    else:
#        z = tf.Variable(np.random.normal(size=(batch_size, hparams.n_z),scale=hparams.stdv,loc=hparams.mean).astype('float32'), name='z')
...
def get_neg_KL(hparams,z_mean,z_log_sigma_sq):
    gauss_sigma_sq = hparams.gauss_sigma_sq
    return -0.5 * tf.reduce_sum(1 + z_log_sigma_sq - tf.square(z_mean)/gauss_sigma_sq - tf.exp(z_log_sigma_sq)/gauss_sigma_sq - tf.log(gauss_sigma_sq), 1)
